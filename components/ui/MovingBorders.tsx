...
export function Button({
  ...
  borderRadius?: string;
  children: React.ReactNode;
  as?: React.ElementType; // Changed to ElementType
  containerClassName?: string;
  borderClassName?: string;
  duration?: number;
  className?: string;
  [key: string]: any; // You can change this to React.HTMLProps<HTMLElement> if you want to restrict it further
}) {
  return (
    <Component
      className={cn(
...
export const MovingBorder = ({
  ...
  rx?: string;
  ry?: string;
  [key: string]: any; // Can be changed to React.SVGProps<SVGSVGElement>
}) => {
  const pathRef = useRef<SVGRectElement | null>(null); // Changed to SVGRectElement
  const progress = useMotionValue<number>(0);
